...
<?php
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Quantity</th>
          <th>Total</th>
          <th>Gift Message</th> <!-- New header -->
          <th>Gift Wrapping</th> <!-- New header -->
          <th>Action</th>
  ...
  const renderCart = () => {
    ...
    <tr>
        ...
        <td>
          <input type="number" value="${product.quantity}" min="1" class="quantity-input" data-index="${index}" style="width: 60px; text-align: center;">
        </td>
        <td>Rs. <span>${total}</span></td>
        <td>${product.giftMessage}</td>  <!-- New column for Gift Message -->
        <td>${product.giftWrapping}</td> <!-- New column for Gift Wrapping -->
        <td>
...
<script>
  document.getElementById("checkoutButton").addEventListener("click", function(event) {
    ...
    let summary = cart.map(item => `${item.name} (x${item.quantity}) - Rs. ${item.price * item.quantity}\nGift Message: ${item.giftMessage || "None"}\nGift Wrapping: ${item.giftWrapping || "No"}`).join("\n");
    let totalAmount = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0).toString();

    let confirmMessage = `Order Summary:\n\n${summary}\n\nTotal: Rs. ${totalAmount}\n\n>Proceed to Payment?`;

    if (confirm(confirmMessage)) {
    // Store the orders before redirecting to Google Forms
        let processedOrders = JSON.parse(localStorage.getItem("processedItems")) || [];
        processedOrders.push(...cart);
        localStorage.setItem("processedItems", JSON.stringify(processedOrders));
